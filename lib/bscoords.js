...
// reg expression to fetch coordinates from cell2gps response
const RE_FETCH_CELL2GPS_LAT  = /location\s+is\s+\(([+\-\d\.]+),[+\-\d\.]+\)\s+Accuracy/i;
const RE_FETCH_CELL2GPS_LON  = /location\s+is\s+\([+\-\d\.]+,([+\-\d\.]+)\)\s+Accuracy/i;
...
const init = options => {
    ...
    API_KEY_YANDEX    = options.apikey_yandex      || API_KEY_YANDEX;
...
SERVICES['yandex'] = (countrycode, operatorid, lac, cellid) => {
    ...
    const options = {
        protocol: 'http:',
        hostname: 'api.lbs.yandex.net',
        method  : 'POST',
        path    : '/geolocation',
        headers : {}
    };

    const request_body = `json={
                            "common": {
                                "version": "1.0",
                                "api_key": "${API_KEY_YANDEX}"},
                                "gsm_cells": [
                                    { "countrycode": ${countrycode}, "operatorid": ${operatorid}, "cellid": ${cellid}, "lac": ${lac} }
                                ]
                            }`;
    ...
    const response_parser = buf => {
        ...
        try {
            ...
            if (info.position.precision > 10000) {
                return null;
            }
...
SERVICES['cell2gps'] = (countrycode, operatorid, lac, cellid) => {
    // Example:
    // http://www.cell2gps.com/
    const options = {
        protocol: 'http:',
        hostname: 'www.cell2gps.com',
        method  : 'POST',
        path    : '/',
        headers : {
            'Content-Type': 'application/x-www-form-urlencoded',
            'Referer'     : 'http://www.cell2gps.com/'
        }
    };

    const request_body = `mcc=${countrycode}&mnc=${operatorid}&mobile_type=1&` +
                         `lac=${lac}&cellid=${cellid}&sid=&nid=&bid=`;
    ...
    const response_parser = buf => {
        try {
            const coords = {
                lat: Number(RE_FETCH_CELL2GPS_LAT.exec(buf)[1]),
                lon: Number(RE_FETCH_CELL2GPS_LON.exec(buf)[1])
            };
...
const requestAll = (countrycode, operatorid, lac, cellid,
                    ...
                    weights = {
                        yandex: 1, google: 1, mylnikov: 1, opencellid: 1, mozilla: 1, cell2gps: '1'
                    }) => {
    return new Promise((resolve, reject) => {
...
exports.cell2gps   = SERVICES['cell2gps'];
